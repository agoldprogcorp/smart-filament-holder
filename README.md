...
- Запись профилей на NFC метки через мобильное приложение
- Реальное время синхронизации между устройствами
...
**Рекомендуются NTAG216** (888 байт памяти) - дешевле, проще в использовании и более стабильны при записи.
...
2. Сканирование сети для поиска устройств FD-*
...
**NFC метка не читается**: проверить тип метки (рекомендуются **NTAG213/215/216**). Для MIFARE Classic убедитесь что используется стандартный ключ (FF FF FF FF FF FF). Если запись не работает с первого раза, попробуйте еще раз - это нормально для некоторых меток.
